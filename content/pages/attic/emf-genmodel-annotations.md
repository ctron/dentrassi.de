...
Although it has nothing to do with annotations, it is a powerful instrument during the generation process.

A method name can be suffixed with `Gen` and by that “moved out of the way,” but still have it generated.

Assume you have a method generated, called `init`. You can rename the method to “initGen” and keep (!!) the
`@generated` marker. This will let the generation tools still generate the method `init` but as `initGen`. 
The method `init` is missing then and has to be provided manually. But from this method you can call
`initGen` and, to some degree, influence what happens.
